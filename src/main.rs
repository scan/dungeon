...
use bevy::prelude::*;
use bevy_tilemap::{prelude::*, tilemap::TilemapError};
...
fn animate_sprite_system(time: Res<Time>,
...
fn setup(commands: &mut Commands, asset_server: Res<AssetServer>, mut texture_atlases: ResMut<Assets<TextureAtlas>>) {
  ...
  commands.spawn(Camera2dBundle::default())
          .spawn(SpriteSheetBundle { texture_atlas: texture_atlas_handle,
...
fn setup_tilemap(asset_server: Res<AssetServer>) {
  ...
  if let Ok(tilemap) = Tilemap::builder().texture_atlas(texture_handle).tile_dimensions(32, 32).dimensions(40, 30).finish() {
